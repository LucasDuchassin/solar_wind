...
class FeatureExtractor(BaseEstimator):
    ...
    def transform(self, X):
        ...
        X = compute_rolling_std(X, 'Vth', '2h')
        X = compute_rolling_mean(X, 'Vth', '2h')
        ...
        X = compute_rolling_std(X, 'Vth', '1h')
        X = compute_rolling_mean(X, 'Vth', '1h')

        X = compute_rolling_std(X, "B", "4h")
        X = compute_rolling_mean(X, "B", "4h")
        X = compute_rolling_std(X, 'Beta', '4h')
        X = compute_rolling_mean(X, 'Beta', '4h')
        X = compute_rolling_std(X, 'RmsBob', '4h')
        X = compute_rolling_mean(X, 'RmsBob', '4h')
        X = compute_rolling_std(X, 'Vx', '4h')
        X = compute_rolling_mean(X, 'Vx', '4h')
        X = compute_rolling_std(X, 'Vth', '4h')
        X = compute_rolling_mean(X, 'Vth', '4h')
        ...
        X = compute_rolling_std(X, 'Vth', '6h')
        X = compute_rolling_mean(X, 'Vth', '6h')
        ...
        X = compute_rolling_std(X, 'Vth', '12h')
        X = compute_rolling_mean(X, 'Vth', '12h')

        X = compute_rolling_std(X, "B", "24h")
        X = compute_rolling_mean(X, "B", "24h")
        X = compute_rolling_std(X, 'Beta', '24h')
        X = compute_rolling_mean(X, 'Beta', '24h')
        X = compute_rolling_std(X, 'RmsBob', '24h')
        X = compute_rolling_mean(X, 'RmsBob', '24h')
        X = compute_rolling_std(X, 'Vx', '24h')
        X = compute_rolling_mean(X, 'Vx', '24h')
        X = compute_rolling_std(X, 'Vth', '24h')
        X = compute_rolling_mean(X, 'Vth', '24h')

        X = compute_rolling_std(X, "B", "48h")
        X = compute_rolling_mean(X, "B", "48h")
        X = compute_rolling_std(X, 'Beta', '48h')
        X = compute_rolling_mean(X, 'Beta', '48h')
        X = compute_rolling_std(X, 'RmsBob', '48h')
        X = compute_rolling_mean(X, 'RmsBob', '48h')
        X = compute_rolling_std(X, 'Vx', '48h')
        X = compute_rolling_mean(X, 'Vx', '48h')
        X = compute_rolling_std(X, 'Vth', '48h')
        X = compute_rolling_mean(X, 'Vth', '48h')
        return X
...
def get_estimator():
    ...
    classifier = CatBoostClassifier(iterations=350,
                                    depth=4,
                                    l2_leaf_reg=3,
