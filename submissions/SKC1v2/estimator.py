...
def compute_rolling_diff_1h(X_df, feature):
    """
    For a given dataframe, compute the mean over
    a defined period of time (time_window) of a defined feature
    ...
    center : bool
        ...
        center or end of the window
    """
    name = "_".join([feature, "diff_1h"])
    X_df[name] = X_df[feature].diff(periods=12)
    X_df[name] = X_df[name].ffill().bfill()
    X_df[name] = X_df[name].astype(X_df[feature].dtype)
    return X_df

def compute_rolling_diff_6h(X_df, feature):
    """
    For a given dataframe, compute the mean over
    a defined period of time (time_window) of a defined feature
...
class FeatureExtractor(BaseEstimator):
    ...
    def transform(self, X):
        ...
        for i in Cols:
            for j in ["2h", "6h", "12h", "24h", "48h"]:
                X = compute_rolling_variables(X, i, j)
            X = compute_rolling_diff_1h(X, i)
            X = compute_rolling_diff_6h(X, i)
            X = X.copy()
